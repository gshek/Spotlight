...
function e4test_make_LOGFILE {
    export LOGFILE="logs/ext4/`basename $0 | cut -d\- -f1`-`date +%y%m%d-%H:%M.%S`"
...
function e4test_fuse_mount {
    ...
    if [ -z "$LOGFILE" ]
    then
        ./spotlight $FS $MOUNTPOINT
    else
        ./spotlight $FS $MOUNTPOINT -o logfile=$LOGFILE
    fi
}
...
function e4test_fuse_mount_callgrind {
    ...
    if [ -z "$LOGFILE" ]
    then
        valgrind --tool=callgrind ./spotlight $FS $MOUNTPOINT
    else
        valgrind --tool=callgrind ./spotlight $FS $MOUNTPOINT -o logfile=$LOGFILE
    fi
}
