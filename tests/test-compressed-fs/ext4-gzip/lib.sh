...
function e4test_make_LOGFILE {
    export LOGFILE="logs/ext4-gzip/`basename $0 | cut -d\- -f1`-`date +%y%m%d-%H:%M.%S`"
...
function e4test_fuse_mount {
    ...
    if [ -z "$LOGFILE" ]
    then
        ./spotlight ${FS}.gz $MOUNTPOINT 2>> "$LOGFILE"
    else
        ./spotlight ${FS}.gz $MOUNTPOINT -o logfile=$LOGFILE 2>> "$LOGFILE"
    fi
}
...
function e4test_fuse_mount_callgrind {
    ...
    if [ -z "$LOGFILE" ]
    then
        valgrind --tool=callgrind ./spotlight ${FS}.gz $MOUNTPOINT
    else
        valgrind --tool=callgrind ./spotlight ${FS}.gz $MOUNTPOINT -o logfile=$LOGFILE
    fi
}
