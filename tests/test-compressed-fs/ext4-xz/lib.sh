...
function e4test_make_LOGFILE {
    export LOGFILE="logs/ext4-xz/`basename $0 | cut -d\- -f1`-`date +%y%m%d-%H:%M.%S`"
...
function e4test_make_FS {
    ...
    export FS=`mktemp /tmp/spotlight-test.XXXXXXXX`
...
function e4test_mount {
    mkdir $MOUNTPOINT
    sudo mount -o loop -t $MKE2FS_TYPE $FS $MOUNTPOINT
...
function e4test_fuse_mount {
    mkdir $MOUNTPOINT
    xz --block-size 256KB -k -f -9 -S ".xz" $FS
    if [ -z "$LOGFILE" ]
    then
        ./spotlight ${FS}.xz $MOUNTPOINT 2>> "$LOGFILE"
    else
        ./spotlight ${FS}.xz $MOUNTPOINT -o logfile=$LOGFILE 2>> "$LOGFILE"
    fi
}
...
function e4test_fuse_mount_callgrind {
    ...
    if [ -z "$LOGFILE" ]
    then
        valgrind --tool=callgrind ./spotlight ${FS}.xz $MOUNTPOINT
    else
        valgrind --tool=callgrind ./spotlight ${FS}.xz $MOUNTPOINT -o logfile=$LOGFILE
    fi
}
