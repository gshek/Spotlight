...
static inline int _pread(int fd, void *p, size_t size, off_t where) {
    UNUSED(fd);
    return read_wrapper(disk_file, (uint8_t *) p, where, size);
}
...
{
    disk_file = fopen(path, "r");
    if (disk_file == NULL) {
        return -1;
    }

    disk_fd = fileno(disk_file);
    if (disk_fd < 0) {
        return -errno;
    }
